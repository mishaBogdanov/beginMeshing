...
public:
    ...
    const TriHash&        getTriFromPts        ( size_t                    inPtIdx) { return mPointToTri[inPtIdx]; }
    const TriHash&        getTriFromEdge       ( size_t                    inPtIdx1,
                                                 size_t                    inPtIdx2);
    const TriHash&        getTriFromEdge       ( EdgeNds                   inPtIds);
...
private:
    PointVec                                                                                                                         mPoints;
    EdgeHash                                                                                                                         mEdgesHash;
    std::unordered_map<size_t, TriHash>                                                                                              mPointToTri;
    std::unordered_map<std::array<size_t, 2>, TriHash, OrderedArrayHash<size_t, 2>, OrderedArrayEqual<size_t, 2>>                    mEdgeToTri;
